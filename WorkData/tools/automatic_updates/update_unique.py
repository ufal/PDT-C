...
class Morphology:
    ...
    def process(self, form, lemma, tag):
        ...
        if (lemma, tag) not in analyses:
            ...
            # Only one raw lemma
            rawlemma_matches = [(l, t) for l, t in analyses if self._raw(l) == raw_lemma]
            if len(rawlemma_matches) == 1 and (
                    rawlemma_matches[0][0] == lemma or rawlemma_matches[0][1] == tag):
                replacement = rawlemma_matches[0]

            # Unique analysis
            if len(analyses) == 1:
                replacement = analyses[0]

            if replacement is not None:
                print("{} {} {} -> {} {}".format(
                    form, lemma, tag, replacement[0], replacement[1]))
                return replacement
